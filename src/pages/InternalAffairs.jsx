...
function useStaffSearch() {
  const [target, setTarget] = useState(null); // { discordId, username, avatarHash, nickname }
...
export default function InternalAffairs() {
  ...
  useEffect(() => {
    if (!canAccess) return;
    apiFetch("/promotions/ranks").then(({ ranks }) => {
      ...
      <div className="two-col">
        ...
        <div className="card">
          ...
          <form onSubmit={submitPromotion}>
            ...
            <button className="primary" type="submit" disabled={promoSubmitting}>{promoSubmitting ? "Submitting…" : "Suggest Promotion"}</button>
          </form>
        </div>
      </div>
    </div>
